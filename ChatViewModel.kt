...
class ChatViewModel @Inject constructor(
     ...
     * This matches the C++ logic in llama-jni.cpp calculate_dynamic_n_ctx():
     * - <=1B: 4096 tokens
     * - <=3B: 2048 tokens
     * - >3B: 1024 tokens
     */
    private fun calculateDynamicNctx(modelId: String): Int {
        // Extract model size hint from modelId
        // Common patterns: "llama-3-8b", "qwen2.5-7b", "phi-2", "gemma-2b"
        val sizePattern = Regex("(\\d+(?:\\.\\d+)?)\\s*[Bb]")
        val match = sizePattern.find(modelId.lowercase())
    ...
    /**
     * Calculate token budget based on modelId.
     * Uses dynamic n_ctx calculation to maximize context window usage.
     */
    private fun calculateTokenBudget(modelId: String): Int {
        val nCtx = calculateDynamicNctx(modelId)
        return nCtx - reservedOutputTokens
    }
    ...
    private suspend fun buildPrompt(userInput: String, contextMessages: List<Message>, modelId: String): String {
        ...
        // Inject active memory context (increased from 3 to 5 memories)
        ...
        // Calculate token budget for context messages using dynamic n_ctx
        val tokenBudget = calculateTokenBudget(modelId)
        ...
        Logger.d(Logger.Tags.VM, "buildPrompt: template=$template, model=$modelId, n_ctx=${calculateDynamicNctx(modelId)}, token budget=${tokenBudget}, system=${systemPromptTokens}, userInput=${userInputTokens}, remaining=${remainingBudget}")

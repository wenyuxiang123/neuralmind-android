...
// Backend reference counter for llama_backend_free management
...
Java_com_neuralmind_llama_LlamaJNI_createEngine(JNIEnv* env, jobject thiz) {
    ...
    LOGI("Created new LlamaEngine with ID: %ld (backend_refcount=%d)", (long)engineId, backend_refcount);
    return engineId;
}
...
Java_com_neuralmind_llama_LlamaJNI_loadModel(JNIEnv* env, jobject thiz, jlong engineId, jstring modelPath) {
    ...
    // Fallback: if custom threadpool creation failed, continue without custom affinity
    if (!engine->threadpool || !engine->threadpool_batch) {
        LOGW("Custom threadpool creation failed, using default threading (n_threads=%d)", n_threads);
        if (engine->threadpool) { ggml_threadpool_free(engine->threadpool); engine->threadpool = nullptr; }
        if (engine->threadpool_batch) { ggml_threadpool_free(engine->threadpool_batch); engine->threadpool_batch = nullptr; }
    }
